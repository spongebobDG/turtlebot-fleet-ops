...
사용자는 시계 방향 90도 회전과 방향이 정상이고 충돌과 케이블 이상이 없다고 확인했다. 새
정면의 물리 여유도 30cm 이상 확보한 뒤 30회 LiDAR 최소 여유 `0.672m`에서 10cm를
보호 직진했다.
...
속도: 0.0200 m/s
guard progress: 0.1002 m
guard elapsed: 5.35 s
guard exit: 0
독립 종방향 이동: 0.104507 m
독립 횡방향 이동: -0.000430 m
독립 yaw 변화: -0.179 deg
종료 e-stop: true
종료 /cmd_vel: linear.x=0.0, angular.z=0.0
종료 /safety/cmd_vel_in Publisher: 0
map size: 182 x 156
map known cells: 1168 (4.11%)
map occupied cells: 163
이동 후 정면 30-scan 최소 여유: 0.727 m
```

새 방향으로 센서 위치를 옮기자 known cell은 `618 -> 1168`, occupied cell은
`108 -> 163`으로 늘었다. 제자리 회전과 달리 위치 이동이 새 관측 범위를 만들었다는
시스템 증거다. 사용자는 이 첫 10cm의 실제 이동과 방향이 정상이고 충돌과 케이블 이상이
없다고 확인했다.

## 수동 방향 전환과 새 방향 두 번째 10cm

첫 구간 뒤 공간이 부족해 사용자가 로봇 바퀴를 굴려 손으로 약 90도 회전시켰다. 시스템은
직전 yaw 약 `-89.0deg`에서 다음 명령 직전 약 `+4.0deg`로 약 93도 변화한 것을 기록했다.
위치는 거의 변하지 않았고 bringup·OpenCR·SLAM 재시작이나 USB disconnect 증거는 없었다.
사용자 확인으로 이 변화가 장애가 아니라 공간 확보를 위한 수동 회전이었음이 확정됐다.

수동 회전 뒤 두 번째 10cm 보호 직진의 결과는 다음과 같다.

```text
명령 전 전방 최소 여유: 0.605 m
목표: 0.1000 m
속도: 0.0200 m/s
guard progress: 0.1010 m
guard elapsed: 5.31 s
독립 종방향 이동: 0.105216 m
독립 횡방향 이동: -0.000112 m
독립 yaw 변화: -0.072 deg
종료 e-stop: true
종료 /cmd_vel: linear.x=0.0, angular.z=0.0
종료 /safety/cmd_vel_in Publisher: 0
...
사용자는 실제 두 번째 10cm 이동과 방향이 정상이고 충돌과 케이블 이상이 없다고 확인했다.
후속 읽기 전용 검사에서 live map은 `182 x 156`, unknown 26,812셀, free 1,367셀,
occupied 213셀로 known cell이 1,580셀까지 증가했다. `/map` Publisher는 SLAM Toolbox
하나였고 e-stop `true`, 최종 속도 0, 안전 입력 Publisher 0을 재확인했다. 이전 지도
round-trip 검증용 Map Server 두 개는 `/validation_map*`으로 격리돼 실지도와 충돌하지
않았으며 검사 뒤 정상 종료했다.

바퀴로 굴린 수동 회전은 encoder와 SLAM이 연속 추적했으므로 지도를 즉시 폐기할 근거는
없었다. 그러나 보호 명령 사이의 움직임을 기존 프로세스가 발견하지 못한 것은 안전 공백이다.
그래서 마지막 성공 자세를 로봇별 JSON에 원자적으로 저장하고 다음 실행에서 3cm·5도
연속성을 검사하도록 보강했다. e-stop 해제 직전에는 `motion_in_progress`를 먼저 저장하고
성공한 종점만 이를 해제한다. 따라서 작은 부분 이동·강제 종료·불일치·odom 재시작은 다음
이동을 차단하며, e-stop dry-run과 운영자 확인 없이는 기준을 재등록할 수 없다.

### TB1 무동작 배포 검증

커밋 `b490da8`과 정리 경고를 없앤 `c35bb07`을 TB1에 fast-forward 배포했다.
TB1에서 `fleet_navigation` 81개 테스트가 모두 통과했다. 운영 기준을 임의로 승인하지 않기
위해 `/tmp/tb1-pose-checkpoint-deploy.json`만 사용해 실제 ROS graph를 검증했다.

```text
미등록 체크포인트 dry-run: exit 1, e-stop 해제 전 차단
reset dry-run: PASS, motion_in_progress=false
임시 기준 pose: x=0.477419, y=-0.080854, yaw=3.140391 rad
등록 뒤 일반 dry-run: PASS
시작 편차: translation=0.0000m, yaw=0.12deg
최종 e-stop: true
최종 /cmd_vel: linear.x=0.0, angular.z=0.0
최종 /safety/cmd_vel_in Publisher: 0
임시 체크포인트: 삭제 완료
운영 체크포인트: 미등록
```

세 실행 모두 `dry_run=true`여서 바퀴를 움직이지 않았다. 운영 체크포인트는 현재 실제 방향,
케이블, 주변 여유를 사용자가 다시 확인한 뒤에만 등록한다. 그 전에는 기본 실행이 체크포인트
누락으로 fail-closed 된다.
...
10. **로봇을 들어 옮기거나 끌어 odom에 기록되지 않으면 활성 SLAM 지도를 새로 시작한다.**
11. **ROS 통합 테스트의 절대 topic·service 이름도 패키지 간 격리해야 한다.**
12. **360도 LiDAR의 제자리 회전은 방향을 바꾸지만 반드시 known 영역을 늘리지는 않는다.**
13. **명령 하나가 안전해도 명령 사이의 수동 이동은 별도 연속성 검사 없이는 놓칠 수 있다.**
14. **들어 옮긴 이동과 바퀴로 굴려 odom이 추적한 이동을 같은 것으로 단정하지 않는다.**
...
> 테스트를 포함한 robot workspace 전체 145개 테스트와 저속 실차 guard로 재발 방지를
> 검증했습니다.
...
### 8. 수동 회전 원인을 확인했는데 왜 자세 체크포인트를 추가했는가?

정답: 사람의 의도는 ROS graph에서 관측할 수 없고 각 `supervised_motion` 프로세스는 시작
시점의 odom을 새 기준으로 받아들이기 때문이다. 마지막 성공 자세를 영속 저장해 다음 명령
전과 비교해야 승인되지 않은 수동 이동, 부분 실패, odom 재시작을 e-stop 해제 전에 차단할
수 있다.

### 9. 손으로 돌렸는데 지도를 폐기하지 않은 이유는?

정답: 이번 회전은 바퀴가 구르며 encoder odom에 약 93도로 기록됐고 SLAM의 `map -> base`
자세도 일치했으며 프로세스·USB 재시작 증거가 없었다. 들어서 옮겨 odom에 기록되지 않은
재배치와 달리 motion prior가 연속이었기 때문이다. 다만 다음 명령 전 운영자 검토와 기준
재등록은 필요하다.
...
- [x] 클린 맵 시계 방향 90도 사용자 물리 관찰 확인
- [x] 지도 크기·known·occupied cell 재측정
- [x] 새 방향 첫 10cm guard 시스템 측정
- [x] 새 방향 첫 10cm 사용자 물리 관찰 확인
- [x] 새 방향 이동 후 지도 cell 증가 확인
- [x] 공간 부족에 따른 수동 약 90도 회전 원인 확인
- [x] 새 방향 두 번째 10cm guard 시스템 측정
- [x] 새 방향 두 번째 10cm 사용자 물리 관찰 확인
- [x] 명령 간 odom 자세 체크포인트와 차단 테스트 구현
- [x] robot workspace 전체 145개 테스트 통과
- [x] TB1 `fleet_navigation` 81개 테스트 통과
- [x] TB1 미등록 차단·reset dry-run·일반 dry-run 무동작 검증
- [x] 임시 체크포인트 삭제와 운영 체크포인트 미등록 확인
- [ ] 깨끗한 지도 저장 및 시각 검수
...
1. 현재 실제 방향과 공간을 다시 확인하고 dry-run으로 자세 체크포인트를 등록한다.
2. 다음 이동은 손 회전 대신 보호 회전·직진 명령으로만 수행한다.
3. unknown 보존 임계값으로 지도와 pose graph를 저장하고 자동·시각 품질을 검수한다.
