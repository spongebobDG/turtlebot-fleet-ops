...
종료 interrupt 회귀를 포함한 `navigation_agent` 패키지 테스트 115개가 통과했다. 실차에서는
e-stop 아래 NAVIGATION→IDLE을 세 번 반복해 종료 경계를 좁혔다. 첫 수정은 manual traceback을
없앴지만 agent coroutine warning이 남았고, executor cleanup을 1초로 제한한 두 번째 수정은
traceback 대신 exit code `-2`가 남았다. cleanup이 이미 시작된 뒤의 반복 SIGINT만 무시하도록
보강한 최종 audit에서는 두 노드 모두 `finished cleanly`였으며 traceback, KeyboardInterrupt,
미회수 coroutine, invalid publisher, startup mode-service ERROR와 비정상 종료가 모두 0건이었다.

같은 구간에서 Nav2 Humble의 `planner_server`는 launch가 lifecycle child의 ROS context를 동시에
닫을 때 `context is invalid` transition ERROR 4줄을 남겼다. 로봇 상태와 profile 전환은 정상이고
우리 Python node 종료 잡음과는 분리된 upstream lifecycle 종료 제한사항이다. raw evidence는
숨기지 않으며 계획된 IDLE 전환 시각과 대조한다.
