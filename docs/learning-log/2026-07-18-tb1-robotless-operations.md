...
- Gateway 재시작 시 DB에 남은 `STARTING`·`ACTIVE` task와 navigation agent 재시작 시 남은
  `ACTIVE` task를 자동 `FAILED`로 닫아 이전 목표가 재개되거나 다음 작업을 막지 않게 했다.
...
최종 요구사항 대조에서는 공개 navigation API의 fail-closed 상태가 한 테스트로 모두 드러나지
않는 점과 프로세스 재시작 중 task 소유권 상실을 찾았다. 지도 밖·unknown·occupied와 별도로
유한하지 않은 좌표, offline, ERROR, Nav2·AMCL·safety 미준비, e-stop, 미재무장, 잘못된 command ID를
직접 `409` 또는 `422`로 확인했다. Gateway는 재시작 시 이전 action handle을 복원하지 않고
DB의 비종료 task를 실패로 닫으며, 이 복구를 두 번 실행해도 terminal task는 바뀌지 않는다.
요구사항 대조 테스트를 추가한 첫 Humble 실행에서는 Jammy의 구형 requests 기반
`TestClient`가 최신 httpx의 `content=`
인자를 지원하지 않아 제품과 무관한 테스트 호환성 실패가 났다. raw JSON 전송을 두 버전이
모두 지원하는 `data=`로 바꾸고 같은 422 계약을 유지했다.

## 로컬 검증

Windows에는 ROS 2 Humble이 없어 ROS graph 테스트는 CI로 남겼다. 로컬에서는 다음을 실행했다.

- SQLite·FastAPI·task manager·registry·map 테스트: 39 passed, 1 Windows symlink skip
...
[GitHub Actions run 29601662765](https://github.com/spongebobDG/turtlebot-fleet-ops/actions/runs/29601662765)에서
다음을 모두 통과했다.

- Ubuntu 22.04, ROS 2 Humble, 5개 workspace 패키지 build
- `183 tests, 0 errors, 0 failures, 0 skipped`
...
6. 프로세스 재시작은 DB row가 아니라 action 소유권과 lease의 연속성으로 판단해야 한다.
