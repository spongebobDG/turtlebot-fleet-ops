...
날짜: 2026-07-16
단계: Phase 4
진행 상태: 구현·실차 통신·안전 장애 검증 완료, PR #5 병합 완료
...
### 8. GitHub Actions CI를 추가하고 flaky test를 제거

`.github/workflows/ros2-ci.yml`을 추가해 Ubuntu 22.04와 ROS 2 Humble에서 다음 순서를
PR마다 재현하도록 했다.

1. `rosdep update`와 패키지 의존성 설치
2. `robot`, `control`의 네 패키지 colcon build
3. 격리된 domain 142에서 전체 64개 테스트
4. `colcon test-result --verbose`로 실패 수 검사

CI를 처음부터 한 번에 성공시키지는 않았다. 실패를 단계별로 분리해 다음과 같이
안정화했다.

- 1차: runner rosdep이 `--yes`를 지원하지 않아 `-y`로 수정
- 2차: 새 runner의 rosdep 캐시가 없어 `rosdep update` 명시
- 3차: optional flake8 플러그인과 실제 runner CPU 부하 때문에 4개 실패
- lint: 저장소 루트 `.flake8` 계약을 세 패키지 테스트가 명시적으로 사용
- Agent 테스트: 실제 host 메트릭 대신 정상 값의 `SystemMetrics` 테스트 대역 주입
- 4차: `flake8-class-newline` 한 건을 실제 코드 스타일로 수정
- 5차: `Humble build and test` 전체 단계 성공, 실행 시간 4분 10초

마지막으로 `protect-main` Ruleset에 이 검사를 strict required status check로 등록했다.
이제 CI가 실패하거나 작업 브랜치가 최신 `main` 기준으로 검사되지 않으면 병합할 수 없다.
...
8. 테스트는 실제 runner CPU나 설치된 optional 플러그인에 따라 결과가 달라지면 안 된다.
9. CI가 있다는 사실보다 실패 시 병합을 막는 Ruleset 연결이 중요하다.
...
- [x] Draft PR #5 생성
- [x] GitHub Actions CI 64개 테스트 통과
- [x] `Humble build and test`를 main 필수 검사로 등록
- [x] PR 전체 diff 자체 리뷰
- [x] Ready 전환과 squash merge (`ac0bb16`)
...
### 6. Robot Agent 테스트가 CI에서만 `HIGH_CPU`로 실패한 이유와 해결은?

정답: 테스트가 runner의 실제 CPU 사용률을 읽어 외부 부하에 따라 결과가 변했기
때문이다. 제품 임계값을 느슨하게 만들지 않고, 테스트에서 정상 `SystemMetrics`
대역을 주입해 ROS 메시지 흐름만 결정적으로 검증했다.

## 다음에 할 일

1. Phase 5에서 SLAM·Nav2와 로컬 Safety Watchdog의 명령 경계를 검증한다.
2. 터미널 Goal이 실차에서 검증된 뒤 웹 Action Client를 구현한다.
...
- `febaed1 docs: record Phase 4 service recovery`
- `f7ee7ce ci: add ROS 2 Humble build and test workflow`
- `2af1da4 fix: use portable rosdep install flag`
- `0b49b42 fix: initialize rosdep data in CI`
- `0b9f942 test: make ROS checks deterministic`
- `7630ca7 style: satisfy class newline lint`
- PR #5: `feat: add TB1 real-time web fleet dashboard` (squash merge `ac0bb16`)
