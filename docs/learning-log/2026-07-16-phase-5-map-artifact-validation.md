...
수정본을 TB1에 배포한 뒤 현재 활성 지도를 새 `/tmp` 디렉터리에 다시 저장했다. 이번에는
`--free 0.196 --occ 0.65`를 명시했다.

```text
validate_map: PASS
map size: 182 x 156
resolution: 0.05 m/cell
known cells: 1168 (4.11%)
occupied cells: 163
free cells: 1005
unknown cells: 27224
trinary unknown marker cells: 27224
posegraph: 6,960,322 bytes
data: 37,104 bytes
```

같은 YAML을 별도 lifecycle Map Server로 `/safe_validation_map`에 다시 로드한 결과도
known `1168`, occupied `163`, free `1005`, unknown `27224`로 검사기와 정확히 일치했다.
저장 전후 e-stop은 true, `/cmd_vel`은 0, 안전 입력 Publisher는 0개였다. 이 스냅샷은
검사 기능 검증용 임시 파일이며 최종 Nav2 지도나 Public 저장소 산출물로 사용하지 않는다.
...
- [x] TB1 활성 지도의 임시 저장과 `validate_map` 실차 실행
- [x] 별도 Map Server 재로드에서 cell 의미 보존 확인
- [ ] 매핑 완료 후 최종 TB1 지도 저장
- [ ] RViz 지도 품질 검토
- [ ] AMCL 초기 위치와 수렴 검증
...
- `1160ed8 fix: preserve unknown cells in saved maps`
