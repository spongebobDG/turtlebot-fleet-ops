...
날짜: 2026-07-16
단계: Phase 5 - 실제 지도 작성
진행 상태: 새 기준 시작·보호 이동 완료, SLAM queue 보정 뒤 pose graph 3노드와 지도 확장 검증 완료
...
## pose graph 3노드와 지도 확장 10cm 검증

사용자가 전방 30cm 이상, 케이블 고정, 바퀴 자유와 낙하 위험 없음을 새로 확인한 뒤 다음
10cm 구간을 준비했다. 첫 dry-run은 체크포인트와 현재 yaw 차이가 `5.07deg`로 허용값
`5.00deg`를 0.07도 초과해 이동 전에 차단됐다. 이 시도에서는 e-stop을 해제하거나 모터
명령을 보내지 않았다.

현재 e-stop, 0속도, 입력 Publisher 0과 전방 여유를 다시 확인했고 사용자의 같은 구간 준비
승인이 유효한 상태에서 무이동 체크포인트를 등록했다. 일반 dry-run이 통과한 뒤에만 실제
10cm를 실행했다.

```text
target: 0.1000m
speed: 0.0200m/s
guard progress: 0.1002m
elapsed: 5.31s
odom before: x=0.212255, y=0.023541, yaw=0.477833rad
odom after:  x=0.305183, y=0.072560, yaw=0.483778rad
map before: 118x175, known=1045, free=943, occupied/other=102
map after:  121x187, known=1859, free=1697, occupied/other=162
pose graph sphere nodes: 2 -> 3
pose graph total markers: 5 -> 6
front clearance after motion: 1.136m
final e-stop: true
final /cmd_vel: linear.x=0.0, angular.z=0.0
final safe input Publisher: 0
physical observation: 10cm 실제 이동 정상, 충돌 없음, 케이블 이상 없음
```

known cell이 `1045 -> 1859`로 814개 늘고 pose graph가 3노드가 됐으므로 이번 구간은
이동과 매핑 확장을 모두 통과했다. 다만 출발 지점 재관측에 의한 loop closure는 아직 별도
증거가 없으므로 다음 세션의 첫 매핑 목표로 남긴다.
...
9. **허용값을 아주 조금 넘은 자세 불일치도 먼저 차단하고, 임계값을 넓히지 말고 무이동·현장 승인·안전 상태를 근거로 새 기준을 등록한다.**
...
> 2개로 증가해 SLAM이 이동 스캔을 실제로 채택했음을 확인했습니다. 이어진 10cm 구간에서는
> 사전 자세 검사 5.07도가 5도 한계를 넘어 먼저 차단됐고, 무이동 상태와 현장 승인을 확인한
> 뒤 새 기준을 등록했습니다. 실제 이동 후 pose graph는 3노드, known cell은 1045개에서
> 1859개로 증가해 이동과 지도 확장을 함께 입증했습니다.
...
### 7. yaw 차이가 허용값 5도보다 0.07도만 큰 경우 임계값을 바로 넓혀도 되는가?

정답: 안 된다. 작은 초과라도 이동 전 차단은 정상 동작이다. e-stop, 0속도, 입력 Publisher 0,
사용자의 최신 현장 승인과 무이동 상태를 확인해 실제 기준 변화인지 판정하고, 정당한 경우에만
별도 dry-run으로 체크포인트를 등록한다. 임계값 완화는 충분한 측정 자료가 있을 때 설계 변경과
테스트를 거쳐야 한다.
...
- [x] 다음 10cm 보호 이동과 사용자 물리 관찰
- [x] pose graph 2개에서 3개, known cell 1045개에서 1859개로 증가 확인
- [ ] loop closure 검증
