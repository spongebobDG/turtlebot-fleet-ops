...
진행 상태: 진행 중 - 공중 안전 시험 완료, 바닥 저속 주행 검증 대기
...
주변 안전거리를 확보하고 TB1을 바닥에 내려놓은 뒤 `0.03 m/s` 직진과 `0.2 rad/s`
제자리 회전을 짧게 실행한다. 각 입력 종료 후 0 속도와 `/odom` 위치·방향 변화를
확인한다. 바닥 주행이 끝나면 Phase 2 필수 개념, 복습 문제와 면접 답변을 완성한다.
