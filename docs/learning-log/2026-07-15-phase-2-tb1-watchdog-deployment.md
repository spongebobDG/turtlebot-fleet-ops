...
진행 상태: 진행 중 - 안전 기능과 바닥 저속 주행 완료, keyboard teleop 대기
...
- [ ] keyboard teleop을 안전 입력 토픽으로 remap해 확인

## 다음에 할 일

`turtlebot3_teleop` 패키지와 실행 파일을 확인한다. keyboard teleop의 `cmd_vel`을
`/safety/cmd_vel_in`으로 remap하고 짧은 전진·정지 입력을 확인한다. 완료 후 Phase 2
필수 개념, 복습 문제와 면접 답변을 완성한다.
