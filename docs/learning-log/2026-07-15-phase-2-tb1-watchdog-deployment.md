...
진행 상태: 진행 중 - TB1 빌드·속도 제한·timeout 정지 검증 완료
...
바퀴를 띄운 상태에서 연속 저속 명령을 보내고 emergency stop을 활성화한다. 입력이
계속되는 중에도 0 속도로 정지하는지 확인한다. 정지 해제 뒤에는 연속 비영 명령을
거부하고 중립 명령을 받은 뒤에만 재무장하는지 검증한다.
