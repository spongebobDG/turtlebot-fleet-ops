...
진행 상태: 원격 게시와 Draft PR 생성 완료, 자체 리뷰 대기
...
GitHub의 `Files changed` 화면에서 전체 변경을 자체 리뷰한다. 문서 내용과 검증 결과가 맞으면 Ready for review로 전환하고 squash merge한다.
