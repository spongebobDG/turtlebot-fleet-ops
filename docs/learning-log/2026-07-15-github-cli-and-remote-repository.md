...
진행 상태: 원격 게시, 자체 리뷰와 Ready for review 전환 완료
...
로컬 `main`과 작업 브랜치를 원격에 게시하고 추적 브랜치를 연결했다.

```text
origin/main=b7a03bb
origin/chore/phase-0-dev-environment=186652b
Pull Request=#1
PR state=OPEN
PR mode=Draft
base=main
...
Draft PR 주소: <https://github.com/spongebobDG/turtlebot-fleet-ops/pull/1>
...
- [x] `main` 원격 게시
- [x] 작업 브랜치 원격 게시
- [x] Draft Pull Request 생성
- [x] GitHub `Files changed` 자체 리뷰
- [x] Ready for review 전환
- [ ] Squash merge와 작업 브랜치 정리
...
PR #1은 자체 리뷰 후 Ready for review로 전환했고 GitHub에서 `MERGEABLE`, `CLEAN` 상태를 확인했다. 다음으로 squash merge하고 작업 브랜치를 정리한다.

## 관련 커밋

- `a9c18c4 docs: add GitHub remote and PR workflow`
- `186652b fix: clean up PR template whitespace`
