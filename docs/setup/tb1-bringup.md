...
상태: Phase 1 기본 bringup 완료. GPIO UART 우회 배선과 안정 전원은 매 운용 전
실데이터 기준으로 확인해야 하며, LDS-02 포트 하드코딩은 후속 개선이 필요하다.
...
> `/dev/serial0` 파일과 `/scan` Publisher가 존재하는 것만으로 센서 정상 상태를
> 판정하면 안 된다. 2026-08-18 재검증에서는 UART에 바이트가 들어왔지만 5초 동안
> 정상 `54 2c` 헤더가 1개뿐이었고, Raspberry Pi가 `get_throttled=0x50005`로
> 저전압·throttling 상태였다. 안정적인 5V/3A 이상 전원과 임시 GPIO15 TX/GND
> 점퍼를 먼저 점검한 뒤 정상 패킷과 실제 `/scan` 메시지를 모두 확인해야 한다.
