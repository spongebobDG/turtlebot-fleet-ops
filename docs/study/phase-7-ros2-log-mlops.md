...
### LiDAR endpoint 정합과 AMCL은 어떤 관계인가

초기 pose는 위치 `(x,y)`뿐 아니라 방향 `yaw`가 맞아야 한다. yaw가 180° 뒤집히면 화면에서
로봇이 벽을 통과하는 것처럼 보이고 Nav2는 실제와 반대인 costmap에서 recovery를 반복할 수 있다.
웹의 scan-map 정합은 현재 LiDAR endpoint를 후보 pose로 지도에 투영하고 occupied cell까지의
거리로 점수를 만든다. coarse 전역 탐색 뒤 sub-cell·sub-degree로 보정하면 사람이 대략 찍은
pose의 방향 오류를 줄일 수 있다.

이 과정은 AMCL을 대체하지 않는다. scan matcher는 초기 seed의 명백한 불일치를 거부하고 좋은
후보를 제안하며, AMCL은 그 seed 이후 시간에 따른 확률 위치추정을 담당한다. endpoint 일부는
사람·가구처럼 지도에 없는 물체일 수 있으므로 100% 일치를 요구하지 않고 match·지도 내부 비율과
화면 overlay를 함께 본다. 또한 LiDAR 높이 아래 장애물은 어느 정합 알고리즘도 관측할 수 없다.

### 정합 점수가 높으면 센서의 앞방향도 맞는가

그렇지 않다. scan-map 점수는 점들의 공간 배치가 지도 벽과 겹치는지를 보지만, 센서 frame의
angle 0이 차체 `base_link +X`와 같은지는 별도의 외부각(extrinsic) 계약이다. 센서 축이 180°
틀리면 matcher가 로봇 pose를 180° 돌린 후보로 보상해 높은 점수를 만들 수 있다. 그러면 웹
overlay는 그럴듯해도 실제 로봇은 삼각형의 반대로 움직인다.

따라서 세 층을 독립적으로 검증한다.

1. 메시지 형상: 가변 배열을 고정 360-bin으로 정규화한다.
2. 좌표 계약: 원본 angle 0을 실제 `base_link +X`에 맞추는 센서 외부각을 적용한다.
3. 물리 수용: e-stop 아래 overlay를 맞춘 뒤 아주 짧은 전진에서 실제 차체 앞과 웹 삼각형을 비교한다.

이번 TB1은 2번이 π rad였고 Nav2 정규화 스캔과 Gateway raw-scan overlay에 같은 값을 적용했다.
UI yaw 수학, AMCL pose와 센서 외부각 중 하나만 고치면 서로 다른 소비자가 다시 불일치하므로
설정 테스트와 물리 방향 시험을 함께 유지해야 한다.
...
3. 60초 창 특징과 SHA-256 dataset hash를 만든다.
   이때 검증한 정상 운전의 시작·종료 epoch를 고정해 배포·초기 위치 대기·사고 구간을 baseline에서
   제외하고, 제외 건수도 manifest에 기록한다.
...
| 초기 위치는 free cell이면 충분하다 | free cell과 함께 LiDAR endpoint가 지도 벽에 정합되는지 검사해야 한다 |
...
- 정상 기준: 15분 15초, 13개 창, INFO 77건, dataset hash `49fb354e...b9654ae`
- Production: median/MAD model `ros2-log-mad-2026-07-19T134802.259185+0000-6576983b`
- live 추론: NORMAL, score 9.0, threshold 55.75
- 정상 replay: 5분 구간 3개 모두 NORMAL, false positive 0/3
- 오류 replay: transform timeout과 충돌·제어 지연 모두 ANOMALY, score 80
- 보존: raw 30일, dataset·model·Production registry는 자동 삭제하지 않음

이 수치는 모델 정확도를 일반화한 통계가 아니라 TB1 한 대의 현재 software·지도·운전 조건에
대한 acceptance 표본이다. 설정이나 환경이 바뀌면 새 dataset hash로 drift와 false positive를
다시 측정한다.
...
- [ ] yaw 180° 오류가 Nav2와 웹 표시에 미치는 영향과 scan-map gate의 역할을 설명한다.
- [ ] 지도 드래그의 원은 위치 중심, 삼각형은 `base_link`의 +X 앞방향임을 설명한다.
- [ ] scan-map 점수가 높아도 센서 외부각이 틀릴 수 있는 이유와 물리 전진 시험의 역할을 설명한다.
