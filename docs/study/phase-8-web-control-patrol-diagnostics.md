...
## 14. 정상 SIGINT traceback도 MLOps 데이터 오염이다

`ros2 launch`는 프로필을 정지할 때 child process 전체에 SIGINT를 보낸다. Python 노드가
`rclpy.spin()`의 `KeyboardInterrupt`를 처리하지 않거나, executor보다 node를 먼저 파괴하면 정상
종료인데도 traceback, invalid publisher, 미회수 coroutine 메시지가 남을 수 있다. 모델은 문자열만
보므로 이런 종료 잡음도 restart/error feature를 올린다.

정리 순서는 fail-closed 출력을 시도한 뒤 executor의 callback을 끝내고 node를 파괴하는 순서다.
다만 launch가 context를 먼저 닫을 수 있으므로 각 정리 단계는 `KeyboardInterrupt`와
invalid-context `RuntimeError`를 독립적으로 흡수해야 한다. context가 이미 닫혔을 때 0을 다시
publish하지 않아도 안전한 이유는 authorization 갱신 중단과 watchdog이 별도 로컬 경계에서 이미
정지를 보장하기 때문이다. 정상 종료 잡음을 없애는 것은 오류를 숨기는 작업이 아니라 실제
장애 로그의 신호 대 잡음비를 높이는 작업이다.

TB1 최종 audit에서는 manual control과 navigation agent가 모두 clean exit했고 관련 종료 오류가
0건이었다. 다만 같은 `ros2 launch`의 Nav2 planner lifecycle은 Humble에서 context 종료 경쟁
ERROR를 남길 수 있다. 이 경우 profile 상태가 IDLE로 확인됐는지, systemd가 정상 stop으로
기록했는지, 같은 시각에 작업자가 계획된 profile 전환을 요청했는지를 함께 본다. 이 세 근거가
없다면 정상 종료 잡음으로 단정하지 말고 실제 process crash로 조사한다.
