...
TB1 agent는 Nav2 목표를 소유하며, arbiter, Python watchdog policy와 C++ watchdog guard가
속도 권한을 독립적으로 닫는다.
...
실차 장애 주입에서는 단절 후 2.112초에 최종 0, 2.263초에 `LEASE_EXPIRED`를 측정했다.
재연결 뒤 이전 목표는 재개되지 않았다.
...
처음에는 Python watchdog 하나가 안전 정책과 `/cmd_vel` 발행을 모두 담당했다. 활성 목표
중 이 프로세스를 종료하자 systemd가 다시 띄우기 전까지 `/cmd_vel`의 0 Publisher도 함께
사라졌고, 관찰된 첫 0이 종료 뒤 3.491초에 나타났다. “명령을 발행하지 않음”을 “0 명령을
발행함”과 같게 취급한 설계 결함이었다.

C++ guard를 유일한 `/cmd_vel` Publisher로 두고 Python policy가
`/safety/watchdog_cmd_vel`까지만 발행하게 바꿨다. guard는 policy가 0.25초 끊기면 계속
0을 내고, 속도 clamp와 시작 후 중립 조건도 독립적으로 적용한다. guard가 재시작하면
transient-local 신호로 policy도 `WAITING_NEUTRAL`로 돌아간다. 최종 실차 장애 주입에서
guard 프로세스는 약 0.105초에 재생성됐고 첫 0은 0.955초에 관찰됐으며 목표는 취소되고
자동 재출발하지 않았다.
...
| Python watchdog policy 종료 | C++ guard가 0.25초 입력 timeout으로 0 유지 |
| C++ watchdog guard 시작·재시작 | 비영점 차단, 중립 요구, policy에 재시작 신호 |
...
policy와 guard는 IDLE 0을 새로 받아야 재무장한다. agent의 startup cancel-all은 요청을 보낸
...
## odom 진척과 map 진척은 다르다

Nav2의 기본 progress checker는 로봇이 odom에서 일정 거리 움직였는지를 판단한다. 이는 일반적인
주행 정체 감지에는 유용하지만, 바퀴가 미끄러지거나 `map -> odom` 위치추정이 흔들릴 때는 odom
이동이 있어도 실제 목표 오차가 줄지 않는 상황을 놓칠 수 있다. Action이 계속 `ACTIVE`이고 속도
상한을 지켰다는 사실만으로 목적지 제어가 정상이라는 뜻은 아니다.

이 프로젝트는 navigation agent에서 두 종류의 오차를 함께 본다.

- `distance_remaining`: 경로를 따라 목표까지 남은 거리
- map-frame yaw error: 현재 자세와 목표 자세의 최단 각도 차이

거리 5cm 또는 yaw 0.1rad의 누적 개선을 material progress로 보고 20초 창을 갱신한다. recovery가
시작되면 한 번 새 창을 주되, feedback 3초 단절과 목표 180초 절대 상한은 별도로 유지한다. 이
감시는 Nav2를 대체하는 planner가 아니라 “downstream이 살아 있으나 목표에 수렴하지 않는” 실패를
닫는 로봇 로컬 안전 경계다. 실패 시 authorization을 철회하고 arbiter를 IDLE로 만든 뒤 custom
action을 `FAILED`로 끝낸다.
...
| 틀린 설명 | 정확한 설명 |
| --- | --- |
| Nav2를 remap했으니 안전하다 | arbiter authorization, policy와 guard clamp/timeout까지 통과해야 한다 |
...
| odom이 계속 변하니 목표도 진행 중이다 | map 목표 오차가 줄어드는지 별도로 확인해야 한다 |
| 속도 상한 안이면 오래 움직여도 안전하다 | 진척·feedback·절대시간 상한으로 권한을 회수해야 한다 |
...
0.5초 안에 갱신될 때만 watchdog policy 입력으로 전달됩니다. Python policy는 0.5초
timeout과 안전 상태를 적용하고, C++ guard가 다시 0.25초 timeout과 0.05 m/s, 0.3 rad/s
상한을 적용해 `/cmd_vel`을 단독 발행합니다. e-stop은 policy를 먼저 정지시킨 뒤 action과
lease를 취소하며, guard·policy·agent가 재시작해도 중립, IDLE과 cancel-all 확인에서
시작합니다. Nav2 action
...
13. Zenoh 건너편에서 상태 topic만 수신한 것으로 action 전달을 검증했다고 할 수 없는
    이유는 무엇인가?
14. watchdog policy가 죽었을 때 `/cmd_vel`을 발행하지 않는 것만으로 부족한 이유는
    무엇인가?
15. C++ guard의 재시작 신호를 transient-local로 발행하는 이유는 무엇인가?
16. odom progress checker만으로 AMCL 비수렴을 감지하지 못할 수 있는 이유는 무엇인가?
17. progress timeout과 hard goal timeout을 별도로 두는 이유는 무엇인가?
