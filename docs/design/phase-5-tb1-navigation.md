...
Nav2 속도를 만든다. 저장소 소유 non-composed Nav2 launch는 Phase 8부터 controller와
`behavior_server` recovery의 `cmd_vel`을 모두 `cmd_vel_nav`로 보내고 smoother의
`cmd_vel_smoothed`만 `/motion/navigation/cmd_vel`로 remap한다. 공통 group remap을 쓰지
않으므로 smoother 입력과 출력이 합쳐지지 않으며, 정상 주행과 recovery 모두 같은 평활화와
watchdog 경계를 통과한다.
