...
| 5B 지도 작성 | SLAM Toolbox | 전용 저속 모션 가드 | PGM, YAML, pose graph |
...
Fleet Gateway에 로봇별 `NavigateToPose` Action Client와 thread-safe Goal registry를
추가했다. 실차 Goal 버튼은 터미널 검증과 현장 준비가 끝난 뒤 사용한다.
...
POST retry -> failed Goal target copied into a new Goal ID
...
Action 수락이 HTTP 제한시간보다 늦게 도착하거나 Goal 자체 제한시간을 넘기면 e-stop과
취소를 함께 요청한다. 이미 종료된 Goal은 다시 `RUNNING`으로 전이할 수 없고, 이전 Goal의
늦은 result callback은 새 Goal handle을 삭제할 수 없다. 이 두 경합 조건은 registry와
Goal ID 소유권 검사로 차단한다.

새 Goal과 재시도는 로봇 watchdog가 발행한 `/safety/estop_active` heartbeat가 2초 이내의
최신 값이고 `false`일 때만 Action Server로 전달한다. 상태를 아직 받지 못했거나 stale이면
fail-closed로 거부한다. 이 상태 토픽은 transient-local QoS로 Zenoh를 통과하므로 Gateway가
늦게 시작해도 최신 상태를 받고 이후 heartbeat로 liveness를 확인한다.

재시도는 `ABORTED`, `REJECTED`, `TIMEOUT`에서만 가능하다. 이전 target과 timeout은
보존하지만 새 Goal ID를 발급하고 `retry_count`와 `retried_from_goal_id`를 기록한다.
registry lock 안에서 실패 Goal을 새 `PENDING`으로 교체해 동시 재시도 중 하나만 소유권을
얻는다. `SUCCEEDED`와 운영자 `CANCELED`는 자동으로 반복하지 않는다.
