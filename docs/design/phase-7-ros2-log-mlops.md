...
Build 단계는 raw 전체를 지우거나 덮어쓰지 않고 검증한 `[since_timestamp, until_timestamp]`만
...
- 방향 표시는 원을 pose 중심, 선 끝 삼각형을 `base_link` +X 앞방향으로 그려 드래그 방향을
  반대로 해석하지 않게 한다.
- 지도 밖, unknown, occupied 셀은 브라우저에서도 거부하고 서버가 다시 검증한다.
- 휠 확대, Shift+드래그 이동, 커서의 world 좌표·cell·occupancy 값을 제공한다.

초기 위치 모드에서는 별도 `/scan` REST 응답의 base-local endpoint를 현재 후보 pose로 변환해
붉은 점으로 겹쳐 그린다. Gateway의 correlative scan matcher는 OccupancyGrid occupied cell의
...
- `point_count`: 정합에 사용한 대표 LiDAR 점 수

최소 40점, match 35%, 지도 내부 70%, score 0.20을 모두 만족해야 적용 버튼이 열린다.
Gateway는 `PUT initial-pose`에서도 같은 점수를 다시 계산하므로 UI를 우회한 뒤집힌 yaw나
...
`MapRegistry.validate_pose`, scan-map 정합 gate와 TB1 로컬 navigation agent에 있다.

추가 HTTP 인터페이스는 다음과 같다.

| API | 동작 |
| --- | --- |
| `GET /api/robots/{id}/scan` | WebSocket과 분리된 최신 base-local LiDAR endpoint 반환 |
| `POST /api/robots/{id}/localization/align-pose` | 대략 pose를 받아 전역 scan-map 보정 후보와 정합 지표 반환 |
...
- [x] false positive와 데이터 보존 기간을 실측한 운영 기준 확정

2026-07-19 실차 수용에서 15분 15초 정상 구간의 13개 창·77건으로 품질 gate를 통과하고
