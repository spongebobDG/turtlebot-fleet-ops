...
| 배터리 | received/fresh/valid, last received, age, percent, voltage | 수신·시간·값 유효성을 분리 |
| 주행 상태 | odom received/fresh/valid, last received, x, y, yaw, velocity | 평면 위치와 속도 snapshot |
| LiDAR | scan received/fresh/valid, last received, age, point count, min range | 원본 scan을 요약한 상태 |
| 시스템 | CPU, memory, disk, load average, uptime, Wi-Fi | Raspberry Pi 자원·통신 상태 |
...
각 source에는 ROS clock 기반 `last_received`와 monotonic 기반 `age_sec`를 함께 제공한다.
전자는 로그·화면에서 실제 시각을 비교하는 용도이고, 후자는 NTP 보정에 영향받지 않는
freshness 판정 용도다.

## Wi-Fi 신호

별도 shell 명령을 반복 실행하지 않고 Linux `/proc/net/wireless`를 읽어 interface,
signal dBm과 0~100 quality를 수집한다. Wi-Fi가 없는 WSL에서는 `wifi_valid=false`와
unknown sentinel을 발행하며, 실제 TB1·TB2의 `wlan0`에서 값을 검증한다.
