...
1. deadman release, 브라우저 종료, Gateway/Zenoh 단절 시 정지 시간
2. 새 환경 매핑, map/pose graph 저장, navigation 재기동과 AMCL 정합
3. 여러 방향의 웨이포인트 순찰, 명시적 취소, 재시작 후 비재개
4. 실제 오류 로그가 incident 원인·근거와 연결되는지 확인
5. 10분 주행 CPU·메모리와 watchdog 단일 `/cmd_vel` Publisher 확인

남은 예상 실차 시간은 새로운 결함이 없을 때 2.5~3.5시간이다. localization 또는 Zenoh 문제를 다시
