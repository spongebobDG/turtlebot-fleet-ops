...
로봇 없는 환경의 구현과 자동 검증은 완료했다. TB1 배포 후 물리 환경에서 방향, 정지 거리,
지도 저장 산출물, 순찰 반복, 장애 복구를 확인하기 전에는 Phase 8을 완료로 표시하지 않는다.
...
Nav2 controller는 10 Hz로 계산하고 `nav2_velocity_smoother`가 20 Hz로 보간한다. 정상
...
다음 항목은 실차에서만 완료할 수 있다.

1. 목표 도착 후 지정 yaw와 실제 차체 전방 일치
2. 20 Hz 평활화의 체감·측정 결과와 장애물 앞 정지
3. deadman release, 브라우저 종료, Gateway/Zenoh 단절 시 정지 시간
4. 새 환경 매핑, map/pose graph 저장, navigation 재기동과 AMCL 정합
5. 여러 방향의 웨이포인트 순찰, 명시적 취소, 재시작 후 비재개
6. 실제 오류 로그가 incident 원인·근거와 연결되는지 확인
7. 10분 주행 CPU·메모리와 watchdog 단일 `/cmd_vel` Publisher 확인

예상 실차 시간은 새로운 결함이 없을 때 3~4시간이다. localization 또는 Zenoh 문제를 다시
