...
로봇 없는 환경의 구현과 자동 검증에 이어 TB1 deadman 단절, 지도 저장·복원, AMCL 재기동,
목적지 yaw·평활화, 순찰 반복·취소·무재개와 600초 자원 시험을 완료했다. 최종 `/cmd_vel`은
safety watchdog 한 노드만 발행하며 모든 이동 시험은 e-stop·미재무장 상태로 종료했다.
...
1. MAPPING에서 map yaml/pgm과 pose graph를 저장하고 validator로 확인한 뒤 운영 원본을 checksum
   그대로 복원했다. NAVIGATION 재기동과 웹 초기 위치 뒤 AMCL ready를 확인했다.
...
로봇을 들어 케이블을 정리한 첫 자원 시험은 216초에 0.176m clearance로 정상 안전 취소됐다.
물리 개입을 제거하고 배터리로 전환한 뒤 수행한 600초 시험만 최종 자원 수용 근거로 사용한다.
