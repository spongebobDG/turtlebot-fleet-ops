...
> 현재 상태: 개발 중. Phase 4는 `main` 병합과 실차 검증을 마쳤고, Phase 5는 2026-07-19
> TB1에서 지도·pose graph 저장, AMCL, 웹 목표, 취소, WARN, e-stop, lease 단절, 프로세스
> 장애와 10분 자원 측정까지 수용 시험을 완료했다. 최신 기준은 격리 Humble 189개와
> TB1 전체 robot workspace 223개 테스트다.
> Phase 6 작업·고장·감사 기능은 robotless 구현을 마쳤지만 TB1 terminal 연동 검증 전이므로
> TB1 전체 MVP는 아직 개발 중으로 둔다.
...
| Phase 6 | TB1 로그·장애 감지·단일 작업 수명주기 | robotless 구현·Humble CI·관제 PC 완료, TB1 terminal 연동 검증 대기 |
...
1. Phase 5에서 기록한 저상 장애물 LiDAR 사각을 운영 전 점검표에 유지하고, 큰 방향 전환
   반복 목표의 recovery·stale 현상은 costmap과 controller tuning 후보로 추적한다.
2. Phase 6 작업·고장·감사 기능을 TB1 terminal과 연결해 생성·실행·취소·실패·재시도와
   systemd 복구를 실차 검증한다.
3. Phase 6 증거와 최종 회귀 CI까지 통과한 뒤 TB1 단일 로봇 MVP 완료를 판정한다.
